<?php $currentPage = basename($_SERVER['PHP_SELF']); ?>
<header id="header" class="full-header" data-sticky-class="not-dark">
    <div id="header-wrap">
        <div class="container clearfix">
            <div id="primary-menu-trigger"><i class="icon-reorder"></i></div>
            <div id="logo">
                <a href="/nl/home" class="standard-logo" data-dark-logo="/images/logo-little.png"><img src="/images/logo-little.png" alt="Groupe Annahda"></a>
                <a href="/nl/home" class="retina-logo" data-dark-logo="/images/logo-little.png"><img src="/images/logo-little.png" alt="Groupe Annahda"></a>
            </div>
            <nav id="primary-menu" class="dark">
                <ul>
                    <li><a href=""><div><img src="../images/icon-translation.png"></div></a>
                        <ul>
                            <li <?= ($currentPage == "index.php") ? 'class="current"' : '' ?> >
                                <a href="/home" title="Français"><img src="/images/icons/flags/french.png" /> Français</a>
                            </li>
                            <li <?= ($currentPage == "index.php") ? 'class="current"' : '' ?> >
                                <a href="/de/home" title="Deutsch"><img src="/images/icons/flags/german.png" /> Deutsch</a>
                            </li>
                            <li <?= ($currentPage == "index.php") ? 'class="current"' : '' ?> >
                                <a href="/nl/home" title="Nederlands"><img src="/images/icons/flags/netherlands.png" /> Nederlands</a>
                            </li>
                            <li <?= ($currentPage == "index.php") ? 'class="current"' : '' ?> >
                                <a href="/es/home" title="Español"><img src="/images/icons/flags/spain.png" /> Español</a>
                            </li>    
                        </ul>
                    </li>
                    <li <?= ($currentPage == "index.php") ? 'class="current"' : '' ?> >
                        <a href="/nl/home"><div>Home</div></a>
                    </li>
                    <li <?= ($currentPage == "portfolio.php" || $currentPage == "portfolio-single-gallery.php") ? 'class="current"' : '' ?> >
                        <a href="/nl/projects"><div>Onze projecten</div></a>
                    </li>
                    <li <?= ($currentPage == "about.php") ? 'class="current"' : '' ?>>
                        <a href="/nl/about"><div>Over Annahda</div></a>
                    </li>
                    <li <?= ($currentPage == "contact.php") ? 'class="current"' : '' ?>>
                        <a href="/nl/contact"><div>Contact</div></a>
                    </li>
                </ul>
            </nav>
        </div>
    </div>
</header>